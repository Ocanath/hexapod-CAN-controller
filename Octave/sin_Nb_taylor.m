function y = sin_Nb_taylor(theta, nout)
...
%     PI_N2 = int32(pi^2*4096);
    ...
    ncoef = 15;
    C_N3 = int32( (pi)^3/factorial(3)*2^ncoef);
...
%     C_N15 = int32( (pi)^15/factorial(15)*2^ncoef);  %1. decays to 0 after this term
    ...
    theta_15b_norm = bitshift(theta*ONE_BY_PI_15B, -12);    %remove the radix12 present in the input. result radix15
    theta2 = bitshift(theta_15b_norm*theta_15b_norm, -15);  %radix15 result
...
%     theta15 = bitshift(theta2*theta13,-15); %max value is 4. must be 0 like feraking alwasy
    ...
    lshift = nout-(15+ncoef);
    %this is getting ridiculous. Should do a lookup table. 
    res = theta*2^(nout-12) - bitshift(theta3*C_N3, lshift);    %theta is radix12, we need to convert to radix_nout
